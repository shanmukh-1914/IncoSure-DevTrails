...
const normalizeBackendClaim = (claim) => {
  const claimDate = claim?.createdAt || new Date().toISOString();

  return {
    id: String(claim.id),
    amount: Number(claim.payoutAmount || 0),
    reason: `Trigger Event #${claim.triggerEventId}`,
    triggerType: "weather",
    status: String(claim.status || "approved").toLowerCase(),
    approvedAt: claimDate,
    payoutAt: claimDate,
    payoutStatus: String(claim.status || "credited").toLowerCase() === "approved" ? "credited" : "pending",
...
export const signUp = async (userData) => {
  ...
  if (isBackendEnabled()) {
    try {
      backendUser = await registerUserApi({
        name,
        email,
        password,
        location,
      ...
      if (/already\s+registered|duplicate|exists/i.test(backendMessage)) {
        return { success: false, error: "User already exists with this email" };
      }

      if (/failed to fetch|networkerror|timeout|abort/i.test(backendMessage)) {
        return {
          success: false,
          error: `Backend registration failed. Ensure backend is running and reachable at ${getApiBaseUrl()}`
        };
      }

      return {
        success: false,
        error: backendMessage || `Backend registration failed. Ensure backend is running and reachable at ${getApiBaseUrl()}`
      };
    }
  }
...
export const login = async (email, password) => {
  ...
  if (isBackendEnabled()) {
    try {
      const backendUser = await loginUserApi({ email, password });
      const users = getAllUsers();
      const existingUser = users.find((u) => String(u.id) === String(backendUser.id));
      ...
      const user = {
        ...
        password,
        role: backendUser.email?.toLowerCase().includes("admin") ? "admin" : "user",
        location: backendUser.location,
        deliveryType: backendUser.deliveryType,
        workingZone: backendUser.zone,
        createdAt: existingUser?.createdAt || new Date().toISOString(),
        insurancePlan: existingUser?.insurancePlan || null,
        activeCoverageDays: existingUser?.activeCoverageDays || 0,
        earningsProtected: existingUser?.earningsProtected || 0,
        lastWeatherCity: existingUser?.lastWeatherCity || backendUser.location
      };

      const nextUsers = existingUser
        ? users.map((u) => (String(u.id) === String(user.id) ? user : u))
        : [...users, user];
      ...
      // Fall back to local auth to preserve demo functionality when backend is unavailable.
    }
  }
...
export const activateInsurancePlan = async (planType) => {
  ...
  if (isBackendEnabled()) {
    try {
      backendPolicy = await createPolicyApi({
        userId: Number(user.id),
        coverageAmount: 600
      });
    } catch (error) {
      return { success: false, error: "Backend policy creation failed. Register a backend user first." };
    }
  }
  ...
  const activePlan = {
    id: `pol_${Date.now()}`,
    type: planType,
    premium: backendPolicy?.weeklyPremium ?? premiumQuote.weeklyPremium,
    basePremium: premiumQuote.basePremium,
    premiumBreakdown: premiumQuote.adjustments,
    coverage: "Income loss only",
    weeklyPremium: backendPolicy?.weeklyPremium ?? premiumQuote.weeklyPremium,
    payoutAmount: backendPolicy?.coverageAmount ?? 600,
    hourlyIncomeProtection: 150,
    activatedAt: new Date().toISOString(),
    expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    status: "active",
    backendPolicyId: backendPolicy?.id ?? null
  };
...
export const getUserClaims = () => {
  ...
  const localClaims = getUserClaimsById(user.id);

  if (!isBackendEnabled()) {
    return localClaims;
  }

  return getClaimsApi(user.id)
    .then((claims) => {
      const backendClaims = claims.map(normalizeBackendClaim);
      const mergedMap = new Map();

      backendClaims.forEach((claim) => {
        mergedMap.set(String(claim.id), claim);
      });

      // Prefer local claim objects when IDs overlap because they carry richer UI metadata.
